  ...
  return (
    <div className="admin-payment-detail">
      ...
      <div className="payment-info-box">
        ...
        <p><strong>결제방식 :</strong> {payment.paymentType}</p>
        <p><strong>결제상태 :</strong> {statusMap[payment.paymentStatus]}</p>
        <p><strong>결제시간 :</strong> {payment.createTime}</p>
        <p> <strong>총 금액 :</strong>{" "} {payment.productPrice?.toLocaleString()} 원 </p>
      </div>
      <h3>결제 상품 목록</h3>
      ...
      <button onClick={() =>
        navigate("/admin/paymentList")} className="back-btn">
        목록으로 돌아가기 </button>
    </div>
  );
};
