...
const DELIVERY_STAGES = [
  { status: "주문 완료", label: "주문/결제 완료" },
  { status: "출고 준비", label: "상품 출고 준비" },
  { status: "운송 중", label: "상품 이동 및 운송" },
  { status: "배송 완료", label: "도착 및 배송 완료" },
];

const DeliveryStatusModal = ({ payment, onClose }) => {
  const currentStatus = payment.deliveryStatus || "주문 완료";
      ...
      >
        ...
        <div className="modalBody">
          <p className="currentStatusTag">
            현재 상태:{" "}
            <span
              className={`status-${currentStatus.replace(/\s/g, "")}`}
            >
              {currentStatus}
            </span>
          </p>
                ...
                <li key={stage.status} className={statusClass}>
                  ...
                  <div className="timelineDetails">
                    ...
                    <span className="timelineStatus">
                      {isCompleted ? "✅ 완료됨" : isCurrent ? "📍 진행 중" : "⌛ 예정"}
                    </span>
